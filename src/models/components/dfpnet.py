"""DfpNet architectures for fluid simulations."""

import torch
import torch.nn as nn
import torch.nn.functional as F

from models.components.unet import SinusoidalPositionEmbeddings


def blockUNet(in_c, out_c, name, transposed=False, bn=True, relu=True, size=4, stride=2, pad=1, dropout=0.):
    """Create a basic UNet block."""
    block = nn.Sequential()
    if relu:
    ...
    if not transposed:
        ...
        block.add_module('%s_upsam' % name, nn.Upsample(scale_factor=stride, mode='bilinear'))
...
class BlockUNetTimeEmb(nn.Module):
    """UNet block with optional time embedding."""
    ...
    def forward(self, x, time_emb):
        """Apply block with time embedding.
...
class DfpNet(nn.Module):
    """U-Net style network for fluid prediction."""
...
class DfpNetTimeEmbedding(nn.Module):
    """Time-conditioned variant of ``DfpNet``."""
    ...
    def forward(self, x, time):
        """Forward pass using a time embedding.
