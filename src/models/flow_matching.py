...
class FlowMatchingModel(pl.LightningModule):
    ...
    def compute_vector_field(self, conditioning: torch.Tensor, data: torch.Tensor):
        """Compute vector fields for training.

        conditioning, data: [B, S, C, H, W]
        """
        if self.dimension == 3:
        ...
        assert len(data.shape) == 5, f"Expected 5D tensor [B, S, C, H, W], got {data.shape}"
        assert len(conditioning.shape) == 5, f"Expected 5D tensor [B, S, C, H, W], got {conditioning.shape}"
        assert data.shape[1] == 1, f"Expected single target frame, got {data.shape[1]} frames"
    ...
    def generate_samples(
        ...
        """Generate a single frame from ``conditioning``.
        ...
        x_0 = torch.randn(batch_size, self.target_channels, conditioning.shape[3], conditioning.shape[4], device=device)
    ...
    def forward(
        ...
        """
        if data is not None:
            return self.compute_vector_field(conditioning, data)
        else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        v_t_target, v_t_pred, cond_target, cond_pred = self.compute_vector_field(conditioning, target)
